...
class KNearestNeighbor(object):
  ...
  def compute_distances_two_loops(self, X):
    ...
    for i in xrange(num_test):
      for j in xrange(num_train):
        ...
        dists[i,j] = np.sum(np.dot(diff,diff)) 
        #####################################################################
        #                       END OF YOUR CODE                            #
        #####################################################################
    return dists
  ...
  def compute_distances_no_loops(self, X):
    ...
    '''
    X_test_sqr = X * X
    X_train_sqr = self.X_train * self.X_train
    X_train_test = np.dot(X , self.X_train.T)
    a = np.sum(X_test_sqr, axis=1)[:, np.newaxis]
    b = np.sum(X_train_sqr, axis=1)[np.newaxis, :]
    c = 2 * X_train_test
    d = a + b
    print a.shape
    print b.shape
